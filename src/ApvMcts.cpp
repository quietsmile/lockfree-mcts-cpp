...
ApvMcts::ApvMcts(std::shared_ptr<Threads> pool, uint8_t threads, uint64_t timePerActionMillis, uint64_t maxIterations):
    pool(pool),
    threads(threads), 
...
uint8_t ApvMcts::getLastAction() { return lastAction; }

uint64_t ApvMcts::getTotalIteration() { return totalIterations.load(); }

void ApvMcts::setRoot(uint8_t action, State* state) {
    if (root) {
...
void ApvMcts::growTree(std::mt19937& random) {
...
Node* ApvMcts::selectOrExpand() {
    Node* node = root;
    while (!node->isTerminal()) {
        // select a node according to u + Q where
        // u = c*P*sqrt(father)/(1+visit)
        // Q = w/visit
        // Node* selectNode = node->select();
...
State* ApvMcts::simulate(Node* node, std::mt19937& random) {
...
void ApvMcts::backPropagate(Node* node, State* terminalState) {
    while (node) {
